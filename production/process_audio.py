...
PAUSE_BANDS = [
    (0.50, 0.22, 0.36),   # 문장 끝 호흡
    (0.28, 0.13, 0.23),   # 쉼표·구 경계
    (0.16, 0.07, 0.13),   # 약한 끊김
    (0.00, 0.035, 0.075), # 어절 사이 — 너무 줄이면 단어가 붙어 뭉개진다
]
TARGET_RATE = 6.10       # 말할 때 음절/초 — 네 구간이 모두 이 값으로 수렴한다
PAUSE_SHARE = 0.16       # 쉼이 전체에서 차지하는 비율
XFADE = int(0.004 * 48000)          # 이어붙일 때 클릭음 방지
EDGE = 0.06              # 앞뒤로 남길 여백
# 템포를 구간마다 크게 다르게 주면 음색이 달라져 중간에 목소리가 바뀐 것처럼 들린다.
# 속도를 완벽히 맞추기보다 네 구간이 같은 처리를 받도록 폭을 좁게 묶는다.
# 구간별 원본 속도가 5.49~6.60 으로 벌어져 있어, 모두 같은 속도로 맞추려면
# 느린 쪽은 당기고 빠른 쪽은 늦춰야 한다. 1.0 기준 편차는 최대 8~11%로
# 어느 구간도 심하게 가공되지 않는다.
TEMPO_MIN, TEMPO_MAX = 0.92, 1.13
